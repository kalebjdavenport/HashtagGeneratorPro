...
**Tradeoffs:**
- Developers must understand the `"use client"` boundary and avoid accidentally pulling server-only code into client bundles
- Content changes to static sections require a rebuild and redeploy

**DDIA connection:** This mirrors the principle of separating read-optimized paths from write-optimized paths. The static layer is a read-optimized materialized view — precomputed at build time, served at the edge. The API layer is the write path, handling dynamic computation on demand.
